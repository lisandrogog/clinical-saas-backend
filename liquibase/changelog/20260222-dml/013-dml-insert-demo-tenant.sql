...
--changeset lisandro.ortega:tenant.003.2 context:dev,qa,prod
...
--changeset lisandro.ortega:business_unit.003.2 context:dev,qa,prod
...
--changeset lisandro.ortega:business_unit.004.2 context:dev,qa,prod
...
--comment: Insert demo tenant users -- Admin123.

INSERT INTO public.app_user
(id, business_partner_id, role_id, status_id, username, password_hash)
VALUES
('019c6e57-c9c4-72d5-b486-e3c0028c8e95'::uuid, 'd5617a2a-386c-476f-9e07-b1bc9f0dc7a9'::uuid, '019c6e4a-2713-720c-830d-8f49c164176a'::uuid, 2, 'admin.tenant', '$2b$10$Rp3NKPCWhnbPr7ADk4681.Q7xh2cYkpAgCXn73v2MaCxD6NfWsQV6')
on conflict (id) do nothing;

--rollback DELETE FROM public.app_user WHERE id = '019c6e57-c9c4-72d5-b486-e3c0028c8e95';

--changeset lisandro.ortega:app_user.004.1 context:dev,qa,prod
--comment: Insert demo tenant users -- Unit123.

INSERT INTO public.app_user
(id, business_partner_id, role_id, status_id, username, password_hash)
VALUES
('019c6e57-c9f5-72f6-ad26-daa3d1cdce2b'::uuid, 'a2a85e3f-c483-4743-bdcb-9b59b7eda59f'::uuid, '019c6e4a-2715-7876-82ab-cc0f7553cb45'::uuid, 2, 'admin.unit', '$2b$10$PaFnyLSZkJHl/2XZaXntg.IJ9snZ3wzC4bkpkicQ88WIv4Ely76s2')
on conflict (id) do nothing;

--rollback DELETE FROM public.app_user WHERE id = '019c6e57-c9f5-72f6-ad26-daa3d1cdce2b';

--changeset lisandro.ortega:app_user.005.1 context:dev,qa,prod
--comment: Insert demo tenant users -- Opera123.

INSERT INTO public.app_user
(id, business_partner_id, role_id, status_id, username, password_hash)
VALUES
('019c6e57-ca26-7424-86c7-9e7f2007dfa2'::uuid, '1c255d79-7594-4584-9f1b-1b91aed9225e'::uuid, '019c6e4a-2717-743d-9493-4a269eca77ac'::uuid, 2, 'operator.unit', '$2b$10$zEnL/K38lH1pV61MLUiAMObpRSAMfLHM67tD1GWGxbH8p1bwGazzy')
on conflict (id) do nothing;

--rollback DELETE FROM public.app_user WHERE id = '019c6e57-ca26-7424-86c7-9e7f2007dfa2';

--changeset lisandro.ortega:app_user_business_unit.003.2 context:dev,qa,prod
--comment: Insert demo tenant app user business units

insert into app_user_business_unit (app_user_id, business_unit_id) 
select app_user_id, business_unit_id from(
	values
...
)as users(app_user_id, business_unit_id)
where not exists (
select 1 from app_user_business_unit aubu 
where aubu.app_user_id = users.app_user_id 
and aubu.business_unit_id = users.business_unit_id
) 
order by app_user_id, business_unit_id;
